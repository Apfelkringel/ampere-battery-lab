...
- Upload workflow regression found (2026-09-23): schedule-triggered runs have no `workflow_dispatch` inputs, so the deployed workflow used fallback version `0.487` and status `completed` while the manual defaults said `0.488` and `draft`. Runs `35826528277` and `35857598086` failed at the initial public-bundle version check (`RELEASE_VERSION: 0.487`); the upload step was never reached and Play state was not mutated. The local correction aligns both runtime fallbacks to version `0.488` and status `draft`; after publishing this change, verify a fresh scheduled run passes bundle/hash validation and still obeys the key activation preflight.
