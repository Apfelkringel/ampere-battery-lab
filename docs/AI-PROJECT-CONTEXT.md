...
- The 31 saved changes resurfaced in the unsubmitted review queue again. The German short-description entry proposed the stale en-dash text `Batterienutzung, Ladestatus und Verlauf – lokal, transparent, ohne Konto.`, not the synchronized source's approved em-dash wording. Re-applied `Für später speichern`; Console confirmed `31 Änderungen für später gespeichert` and the review queue returned to its empty state. Nothing was submitted or published. Reconcile the stale drafts against `store/google-play/listing/short-description.txt` before intentionally submitting translation changes.
