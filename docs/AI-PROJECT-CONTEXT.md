...
- Track-release replacement guard (2026-09-23): Play Console shows a saved internal draft `0.384`/code `467`. The exact pinned `r0adkll/upload-google-play` action sends `tracks.update` with a release array containing only the newly uploaded bundle, so other releases are not preserved in that request. The workflow now detects different existing version codes and skips by default. A manual `allow_track_release_replacement=true` is required only after reviewing the complete track state and tester-continuity impact; scheduled runs cannot provide that override. The legacy draft was not altered. YAML parsing and jq conflict/empty/already-present cases passed locally. Validation-only run `35874530525` on commit `003cfb9` succeeded and emitted the expected skip for existing code `467`; no bundle upload occurred and the Console draft remains unchanged.
