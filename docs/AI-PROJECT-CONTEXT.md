...
- Review of the exact pinned upload action found a supported `versionCodesToRetain` input; its implementation appends retained codes to uploaded codes before writing the draft release. The workflow now uses this only when the requested status is `draft` and all conflicting track releases are also drafts. Non-draft conflicts remain blocked by default; the manual override still requires a deliberate continuity review.
- This retains the previous bundle code in the new draft but does not preserve it as a separate release object or guarantee other per-release metadata. After pushing the workflow change, run `validate_only=true` before key activation and confirm the log says code `467` will be retained while the upload remains skipped. Do not manually upload or replace Alpha/Production releases.
