...
- Upload workflow hardening (2026-09-22): the manual-dispatch version default/example now matches verified public manifest `0.487`; a shared concurrency group serializes scheduled/manual runs, and a 15-minute job timeout prevents stale jobs from overlapping future attempts. The default track remains `internal`.
